...
def global_token_auth(username, password, provider, host):
    ...
    try:
        ...
        logger.debug(f"{host} - No existing auth token found; obtaining a new one.")
...
def rediscover_and_reimport_devices(device_list):
    ...
    for current_device in device_list:
        ...
        logger.info('{} - modules provisioned and previously discovered: {}'.format(current_device['hostname'],device_modules_provisioned))
...
if __name__ == '__main__':
    ...
    if args.target is None:
        ...
        logger.info('Targets specified: {}'.format(args.target))
        targets = args.target
